...
class Ring {
  constructor(radius, orientation, offset) {
    this.radius = radius;
    this.orientation = orientation; // 'x-y', 'y-z', 'x-z'
    this.offset = offset; // To shift the ring along a specific axis
  }

  // Check if a given point (x, y, z) is within the ring
  isPointInRing(x, y, z) {
    ...
    if (orientation === 'x-y') {
      ...
      distance = Math.sqrt((y - offset.y) ** 2 + (z - offset.z) ** 2);
...
// Populate the data array with the number of rings passing through each voxel
